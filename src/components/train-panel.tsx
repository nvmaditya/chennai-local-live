...
import { prevStations } from "@/lib/transit/catalog";
import type { LiveTrain } from "@/lib/transit/types";
import { CrowdBars, RakeMap } from "./rake-map";
import { LINE_COLOUR } from "@/lib/transit/catalog";
...
export function TrainPanel({ train, onClose }: { train: LiveTrain; onClose: () => void }) {
  ...
  return (
    <aside className="flex h-full max-h-[min(70dvh,640px)] flex-col border-t border-border bg-surface md:max-h-none md:border-l md:border-t-0">
      <div className="flex items-start justify-between gap-3 border-b border-border px-4 py-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-elevated px-2 py-0.5 text-[10px] uppercase tracking-wider text-muted">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
        <div className="grid grid-cols-2 gap-2 text-sm">
          <Stat label="Last" value={`${last?.name ?? train.lastReportedStation} · ${train.lastEvent.toLowerCase()}`} />
        ...
        <p className="text-xs text-muted">
          Platform is a geometry default unless a live board overwrites it. Not guaranteed.
        </p>
        ...
        <p className="text-sm leading-relaxed text-fg">{crowd.sentence}</p>
        <p className="text-[11px] uppercase tracking-wider text-subtle">Likely load · not live CCTV</p>
